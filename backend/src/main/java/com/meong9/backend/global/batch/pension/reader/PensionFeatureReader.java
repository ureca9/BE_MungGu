...
@Component
@Slf4j
public class PensionFeatureReader implements ItemReader<CreatePensionFeatureDto> {

    private List<Long> topPensionIds; // topPensionIds를 필드로 선언
    private List<Long> topFeatureIds; // topFeatureIds를 필드로 선언
    private Iterator<CreatePensionFeatureDto> iterator;
    ...
    public void initialize(StepExecution stepExecution) {
        ...
        this.topPensionIds = getListFromContext(jobContext, "topPensionIds");
        this.topFeatureIds = getListFromContext(jobContext, "topFeatureIds");

        // 데이터 검증
        if (topPensionIds == null || topPensionIds.isEmpty() || topFeatureIds.isEmpty()) {
        ...
        log.info("PensionFeatureReader 초기화 완료. topPensionIds: {}, topFeatureIds: {}", topPensionIds, topFeatureIds);

        // CreatePensionFeatureDto 리스트 생성
        List<CreatePensionFeatureDto> features = new ArrayList<>();
        for (int i = 0; i < topPensionIds.size(); i++) {
            features.add(new CreatePensionFeatureDto(topPensionIds.get(i), topFeatureIds.get(i)));
        }
    ...
    @Override
    public CreatePensionFeatureDto read() {
        if (iterator != null && iterator.hasNext()) {
            CreatePensionFeatureDto nextFeature = iterator.next();
